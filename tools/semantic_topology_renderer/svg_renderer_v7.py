"""
HACR Hybrid Observatory — SVG Renderer v7

Continuity climate-front semantic renderer.

Invariant:
continuity climate visibility != prediction
"""
...
def arc_path(radius, start_deg, end_deg):
    ...
    return (
        ...
        f"A {radius} {radius} 0 {large_arc} 1 {end[0]:.2f} {end[1]:.2f}"
    )
...
def render_climate_front(svg, front):
    ...
    svg.append(
        ...
        f'stroke-width="18" opacity="{front["opacity"]}" '
        f'stroke-linecap="round" />'
    )
    svg.append(
        f'<path d="{path}" fill="none" stroke="{front["color"]}" '
        f'stroke-width="4" opacity="{min(front["opacity"] + 0.25, 0.7)}" '
        f'stroke-linecap="round" />'
    )
...
def render_current(svg, positions, current):
    ...
    svg.append(
        ...
        f'stroke="{current["color"]}" stroke-width="18" opacity="0.10" '
        f'stroke-linecap="round" stroke-linejoin="round" />'
    )
    svg.append(
        f'<polyline points="{point_string}" fill="none" '
        f'stroke="{current["color"]}" stroke-width="6" opacity="0.46" '
        f'stroke-linecap="round" stroke-linejoin="round" />'
    )
...
def render_svg(topology):
    ...
    svg.append(
        ...
        f'HACR Continuity Climate Fronts</text>'
    )
    svg.append(
        f'<text x="{CENTER_X}" y="86" fill="#93c5fd" font-size="18" '
        f'text-anchor="middle" font-family="Arial, sans-serif">'
        f'Observer-Only • Non-Authoritative • Climate Front Visibility • UNKNOWN → HOLD</text>'
    )
    svg.append(
        f'<text x="{CENTER_X}" y="112" fill="#cbd5e1" font-size="13" '
        f'text-anchor="middle" font-family="Arial, sans-serif">'
        f'Continuity climate visibility is observational only. It is not prediction, governance, or operational truth.</text>'
    )
    ...
    svg.append(
        f'<rect x="{legend_x}" y="{legend_y}" width="710" height="245" '
        f'fill="#020617" stroke="#334155" stroke-width="1.5" rx="14" />'
    )
    svg.append(
        f'<text x="{legend_x + 22}" y="{legend_y + 36}" fill="white" '
        f'font-size="18" font-family="Arial, sans-serif">Continuity Climate Legend</text>'
    )
    ...
    legend_lines = [
        ...
        "CONGESTED / TURBULENT / STABLE / DIFFUSED / CALM are observational states only",
        "Climate visibility does not predict behavior or certify risk",
        "Continuity climate visibility != prediction",
    ]
    ...
    note_x = SVG_WIDTH - 765
    note_y = SVG_HEIGHT - 290
    svg.append(
        f'<rect x="{note_x}" y="{note_y}" width="710" height="245" '
        f'fill="#020617" stroke="#334155" stroke-width="1.5" rx="14" />'
    )
    svg.append(
    ...
    notes = [
        "Fronts are semantic field descriptors only.",
        "No front predicts operational outcome.",
        "No basin certifies truth, risk, legitimacy, or priority.",
    ...
    svg.append(
        ...
        f'text-anchor="middle" font-family="Arial, sans-serif">'
        f'Bounded continuity climate visibility only. Continuity climate visibility != prediction.</text>'
    )
...
def main():
    ...
    out_file = output_dir / "semantic_topology_v7.svg"
