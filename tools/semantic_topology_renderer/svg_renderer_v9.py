"""
HACR Hybrid Observatory — SVG Renderer v9

Continuity ecology renderer.

Invariant:
continuity ecology visibility != prediction
"""
...
INFLUENCE_MULTIPLIER = {
    "tooling_region": 2.2,
    "governance_observability_region": 1.9,
    "documentation_region": 1.6,
    "boundary_layer": 1.5,
}
...
def render_svg(topology):
    ...
    svg.append(
        ...
        f'text-anchor="middle" font-family="Arial, sans-serif">HACR Continuity Ecology</text>'
    )
    svg.append(
        f'<text x="{CENTER_X}" y="86" fill="#93c5fd" font-size="18" '
        f'text-anchor="middle" font-family="Arial, sans-serif">'
        f'Observer-Only • Non-Authoritative • Continuity Ecology Visibility • UNKNOWN → HOLD</text>'
    )
    svg.append(
        f'<text x="{CENTER_X}" y="112" fill="#cbd5e1" font-size="13" '
        f'text-anchor="middle" font-family="Arial, sans-serif">'
        f'Ecology visibility is observational only. It does not imply prediction, causality, governance, or operational truth.</text>'
    )
    ...
    # Basin influence fields and density fog
    for region in regions:
        ...
        multiplier = INFLUENCE_MULTIPLIER.get(region_id)
        if multiplier:
            svg.append(
                f'<circle cx="{x}" cy="{y}" r="{radius * multiplier:.2f}" '
                f'fill="#94a3b8" opacity="0.055" />'
            )
    ...
    legend_x = 55
    legend_y = SVG_HEIGHT - 305
    svg.append(
        f'<rect x="{legend_x}" y="{legend_y}" width="780" height="260" '
        f'fill="#020617" stroke="#334155" stroke-width="1.5" rx="14" />'
    )
    svg.append(
        f'<text x="{legend_x + 22}" y="{legend_y + 36}" fill="white" '
        f'font-size="18" font-family="Arial, sans-serif">Continuity Ecology Legend</text>'
    )

    legend_lines = [
        "Influence fields show environmental reach only",
        "Density fog shows visibility thickness only",
        "Braided currents show traversal diversity only",
        "Front deflection shows bounded field interaction only",
        "Ecology rendering does not imply causality",
        "Continuity ecology visibility != prediction",
    ]
    ...
    note_x = SVG_WIDTH - 835
    note_y = SVG_HEIGHT - 305
    svg.append(
        f'<rect x="{note_x}" y="{note_y}" width="780" height="260" '
        f'fill="#020617" stroke="#334155" stroke-width="1.5" rx="14" />'
    )
    svg.append(
    ...
    notes = [
        "Ecology states are semantic terrain descriptors only.",
        "No field, front, basin, fog, or current predicts operational outcome.",
        "No rendering certifies causality, truth, legitimacy, risk, or priority.",
    ...
    svg.append(
        ...
        f'text-anchor="middle" font-family="Arial, sans-serif">'
        f'Bounded continuity ecology visibility only. Continuity ecology visibility != prediction.</text>'
    )
...
def main():
    ...
    out_file = output_dir / "semantic_topology_v9.svg"
